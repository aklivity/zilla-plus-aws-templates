...
interface IotIngestAndControlTopicsontext {
  sessions?: string,
  retained?: string,
  messages?: string
}
...
interface IotIngestAndControlContext {
  ...
  topics: IotIngestAndControlTopicsontext;
...
./kafka-topics.sh --create --bootstrap-server ${confluentBootstrapServers} --command-config client.properties --partitions 3 --topic ${kafkaTopicMqttSessions} --config 'cleanup.policy=compact'
